...
class P_BoneMapping(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.prop(context.scene, "simple_active_column", text="活动合并骨骼列")
        row = col.row(align=True)
        row.operator(O_ImportSimpleExcel.bl_idname, text=O_ImportSimpleExcel.bl_label)
        # 添加按钮
        if context.scene.simple_source_armature:
            box.operator(O_BoneSimpleMapping.bl_idname)



        layout.label(text="复制位置:")
        box = layout.box()
        # 选择源骨架
        box.label(text="选择源骨架:")
        box.prop(context.scene, "source_armature", text="", icon="ARMATURE_DATA")
        # 选择目标骨架
        box.label(text="选择目标骨架:")
        box.prop(context.scene, "target_armature", text="", icon="ARMATURE_DATA")
        # 导入Excel文件按钮和文本框
        col = box.column(align=True)
        row = col.row(align=True)
        ...
        row = col.row(align=True)
        row.operator(O_ImportPosExcel.bl_idname, text=O_ImportPosExcel.bl_label)
        # 添加按钮
        if context.scene.source_armature and context.scene.target_armature:
            box.operator(O_BonePosMapping.bl_idname)
        
        layout.label(text="重命名:")
        box = layout.box()
        # 选择源物体
        box.label(text="选择源物体:")
        box.prop(context.scene, "rename_source_mesh", text="", icon="MESH_DATA")
        # 选择源骨架
        box.label(text="选择源骨架:")
        box.prop(context.scene, "rename_source_armature", text="", icon="ARMATURE_DATA")
        # 选择目标骨架
        box.label(text="选择目标骨架:")
        box.prop(context.scene, "rename_target_armature", text="", icon="ARMATURE_DATA")
        # 导入Excel文件按钮和文本框
        col = box.column(align=True)
        row = col.row(align=True)
        ...
        row = col.row(align=True)
        row.operator(O_ImportRenameExcel.bl_idname, text=O_ImportRenameExcel.bl_label)
        # 添加按钮
        if context.scene.rename_source_armature and context.scene.rename_target_armature:
            box.operator(O_BoneRenameMapping.bl_idname)
...
def register():
    ...
    ########################## Divider ##########################
    bpy.types.Scene.simple_source_armature = bpy.props.PointerProperty(type=bpy.types.Object, poll=Kit.is_armature)
    bpy.types.Scene.source_armature = bpy.props.PointerProperty(type=bpy.types.Object, poll=Kit.is_armature)
    bpy.types.Scene.target_armature = bpy.props.PointerProperty(type=bpy.types.Object, poll=Kit.is_armature)
    bpy.types.Scene.rename_source_mesh = bpy.props.PointerProperty(type=bpy.types.Object, poll=Kit.is_mesh)
    bpy.types.Scene.rename_source_armature = bpy.props.PointerProperty(type=bpy.types.Object, poll=Kit.is_armature)
    bpy.types.Scene.rename_target_armature = bpy.props.PointerProperty(type=bpy.types.Object, poll=Kit.is_armature)
    ########################## Divider ##########################
